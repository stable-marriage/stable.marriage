...
do
		...
		do
				...
				do
					## Uniform
					java -cp ${JAR_PATH} algorithms.BiLS_Old -p $prob -c SEq -n "$n" -m "../../datasets/temp/Umen${i}_n${n}${SUF}" -w "../../datasets/temp/Uwomen${i}_n${n}${SUF}" >> "${OUT_PATH}outU_${n}"
					## Discrete
					java -cp ${JAR_PATH} algorithms.BiLS_Old -p $prob -c SEq -n "$n" -m "../../datasets/temp/Dmen${i}_n${n}_h40${SUF}" -w "../../datasets/temp/Dwomen${i}_n${n}_h40${SUF}" >> "${OUT_PATH}outD_${n}"
					## Gauss
					java -cp ${JAR_PATH} algorithms.BiLS_Old -p $prob -c SEq -n "$n" -m "../../datasets/temp/Gmen${i}_n${n}_h40${SUF}" -w "../../datasets/temp/Gwomen${i}_n${n}_h40${SUF}" >> "${OUT_PATH}outG_${n}"
					## UniformDiscrete
					java -cp ${JAR_PATH} algorithms.BiLS_Old -p $prob -c SEq -n "$n" -m "../../datasets/temp/Umen${i}_n${n}${SUF}" -w "../../datasets/temp/Dwomen${i}_n${n}_h40${SUF}" >> "${OUT_PATH}outUD_${n}"
				done
