...
for j in range(len(x_Axis)):
	...
	while line:
		if "Time=" in line:
			...
			if "PolyMin" in impl_names and impl == "MinEgalitarian": 
				if bal:
					val = (float(tokens[7]) + float(tokens[9])) / 2.0
				else:
					val = float(tokens[parseNum])
			...
			if "PolyMin" in impl_names and impl == "MinRegret": 
				if bal:
					val = (float(tokens[7]) + float(tokens[9])) / 2.0
				else:
					val = float(tokens[parseNum])
	...
	# For PolyMin keep the best values of the two for each instance
	for e, r in zip(egal_list, regret_list):
		partial_data["PolyMin"].append(min(e, r))
