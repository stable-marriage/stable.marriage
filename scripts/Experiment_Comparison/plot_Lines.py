...
for i in range(impl_count):
	...
	for curr_n in x_Axis:
		...
		while line:
			...
			if line.startswith("MinEgalitarian:") and impl_names[i] == "PolyMin" and flag:
				tokens = line.split()
				if bal:
					val = (float(tokens[7]) + float(tokens[9])) / 2.0
				else:
					val = float(tokens[parseNum])
				egal_list.append(val)
				flag = False
		
			if line.startswith("MinRegret:") and impl_names[i] == "PolyMin" and flag:
				tokens = line.split()
				if bal:
					val = (float(tokens[7]) + float(tokens[9])) / 2.0
				else:
					val = float(tokens[parseNum])
				regret_list.append(val)
				flag = False
...
# For PolyMin keep the sum of the two for each instance
idx = impl_names.index("PolyMin")
for e, r in zip(egal_list, regret_list):
	data[idx].append(e + r)
...
for i in range(len(l)):
	...
	if (l[i] == "BiLS_SEq_0.125"): l[i] = "iBiLS"
	if (l[i] == "BiLS_Bal_0.125"): l[i] = "iBiLS"
	if (l[i] == "BiLS_Old_SEq_0.0"): l[i] = "BiLS"
	if (l[i] == "BiLS_Old_Bal_0.0"): l[i] = "BiLS"
